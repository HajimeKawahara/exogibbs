...
set OUTDIR = results/fixed_support_v2_reaudit_gpu
mkdir -p $OUTDIR

echo "== fixed-support v2 post-audit GPU rerun =="
date
nvidia-smi
if ( $status != 0 ) then
...
echo "== full exact-state matrix and support lifecycle through support 128 =="
python $RUNNER \
...
if ( $status != 0 ) then
  echo "ERROR: post-audit GPU verification failed"
  exit 1
endif

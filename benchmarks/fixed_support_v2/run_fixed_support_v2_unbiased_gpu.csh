...
set OUTDIR = results/fixed_support_v2_unbiased_gpu
mkdir -p $OUTDIR

echo "== fixed-support v2 unbiased GPU verification =="
date
nvidia-smi
if ( $status != 0 ) then
...
echo "== source and frozen-matrix preflight =="
python -m py_compile \
  $RUNNER \
...
python $RUNNER \
  ...
  --output-dir $OUTDIR
if ( $status != 0 ) then
  echo "ERROR: frozen baseline or support-matrix preflight failed"
  exit 1
endif

echo "== exact-state small/large matrix and separate lifecycle lane =="
python $RUNNER \
  ...
  --lifecycle-max-rounds 4
if ( $status != 0 ) then
  echo "ERROR: unbiased GPU verification failed"
  exit 1
endif

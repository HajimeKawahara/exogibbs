...
def build_common_gas_setup():
    """Return all eleven lower gas species on the unchanged M1 atom reference."""
    _, expanded = build_setups()
    return subset_setup(expanded.gas_setup, UPPER_SPECIES)
...
def anchored_standards_rt(setup, temperature_k, reference_standards):
    ...
    The output order is SHARED_SPECIES. The same element shift must apply to
    ...
    if tuple(setup.species) != UPPER_SPECIES:
        raise ValueError("The common gas setup must preserve the declared species order.")
    matrix = np.asarray(setup.formula_matrix)
    standard = np.asarray(setup.hvector_func(temperature_k))
    columns = [SHARED_SPECIES.index(name) for name in REFERENCE_ANCHORS]

...
OUTDIR = SCRIPT_DIR / "tmp_grid_check_extended"
EXTENDED_LOGK_FILE = "fastchem/logK/logK_extended.dat"
EXTENDED_ELEMENT_FILE = "fastchem/element_abundances/asplund_2020_extended.dat"


def build_extended_fastchem_setup():
    return fastchem_chemsetup(
        path=EXTENDED_LOGK_FILE,
        species_defalt_elements=False,
        element_file=EXTENDED_ELEMENT_FILE,
        silent=True,
    )
...
def main() -> None:
    ...
    equilibrium_options = EquilibriumOptions(
        epsilon_crit=1.0e-14,
        max_iter=10000,
    )

    OUTDIR.mkdir(exist_ok=True)
    path_exo = OUTDIR / "grid_exogibbs_extended.nc"
    print("== Extended Grid Generation Smoke Check ==")
    ...
    print("Using extended FastChem workflow:")
    print("  - logK file:", EXTENDED_LOGK_FILE)
    print("  - element file:", EXTENDED_ELEMENT_FILE)
    print("  - species_defalt_elements:", False)
    print(
    ...
    print("\n[1/5] Build ExoGibbs-backed extended grid with FastChem verification")
    grid_exo = build_equilibrium_grid(
        ...
        setup_builder=build_extended_fastchem_setup,
    )
    ...
    print("\n[2/5] Build FastChem-backed extended comparison grid")
    ...
    print(f"\n[3/5] Save and load extended ExoGibbs grid: {path_exo}")
    ...
    print("\n[4/5] Validate loaded-grid compatibility against extended setup")
    setup = build_extended_fastchem_setup()
    ...
    print("\n[5/5] Check GridEquilibriumInitializer on a matching extended request")
    ...
    print("\nExtended grid generation smoke check completed successfully.")

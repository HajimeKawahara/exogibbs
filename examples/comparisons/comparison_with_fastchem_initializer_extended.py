"""Compare FastChem and ExoGibbs using a saved extended equilibrium-grid initializer."""
...
def main() -> None:
    ...
    nsol_vector = jnp.array(nsol_vector)
    ...
    print("Extended ExoGibbs solve completed with grid-backed initializer.")
    print("Result shape:", nk_result.shape)

    n_elem = len(chem.elements)
    if len(element_vector) != n_elem:
        raise AssertionError(
            "comparison_with_fastchem_initializer_extended: len(element_vector) must equal len(chem.elements)"
        )
    if list(chem.elements)[-1] != "e-":
        raise AssertionError(
            "comparison_with_fastchem_initializer_extended: ExoGibbs chem.elements must keep e- last"
        )
    if list(chem.species[:n_elem]) != list(chem.elements):
        raise AssertionError(
            "comparison_with_fastchem_initializer_extended: molecule species slice must start at len(chem.elements)"
        )
    ...
    if n_plot < 10:
        ax1.legend()
    ax1.set_title(f"Extended FastChem (solid) and ExoGibbs (dashed): T = {T:g} K")
    ...
    output_path = SCRIPT_DIR / f"comparison_fastchem_exogibbs_initializer_extended_{int(T)}.png"

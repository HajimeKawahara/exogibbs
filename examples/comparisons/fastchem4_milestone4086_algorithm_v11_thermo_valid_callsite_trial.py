"""FC4-M4086 thermo-valid algorithm-v1.1 callsite trial on curated cases."""
...
AUDIT_PATH = RESULTS / "fastchem4_milestone4086_algorithm_v11_thermo_valid_callsite_trial.json"
COMPACT_PATH = RESULTS / "fastchem4_milestone4086_algorithm_v11_thermo_valid_callsite_trial_compact.json"
COMPACT_MD_PATH = RESULTS / "fastchem4_milestone4086_algorithm_v11_thermo_valid_callsite_trial_compact.md"
ENGLISH_GUARD_PATH = RESULTS / "fastchem4_milestone4086_english_only_guard.json"
...
CAMPAIGN_FILES = [
    ROOT / "src" / "exogibbs" / "diagnostics" / "condensate_algorithm_v11_callsite.py",
    ROOT / "tests" / "unittests" / "diagnostics" / "condensate_algorithm_v11_callsite_test.py",
    ROOT
    / "examples"
    / "comparisons"
    / "fastchem4_milestone4086_algorithm_v11_thermo_valid_callsite_trial.py",
    ROOT
    / "tests"
    / "unittests"
    / "presets"
...
def _trial_case(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    callsite_data = callsite.as_dict()
    filtered_state = callsite.reduced_step_report.initial_state
    valid_local = tuple(callsite.filter_report.valid_local_indices)
    ac = inputs["formula_matrix_cond_active"][:, valid_local]
    c = inputs["c"][list(valid_local)]
    r = np.asarray(filtered_state.ln_mk, dtype=np.float64)
    rho = np.asarray(filtered_state.rho, dtype=np.float64)
    filtered_inputs = {
        **inputs,
    ...
    return {
        "case_id": row["case_id"],
        "callsite_report": callsite_data,
        "baseline_components": baseline,
        "selected_candidate": selected,
        "accepted_candidate_count": len(accepted),
        "accepted_by_m4086_policy": bool(selected in accepted),
    }


def build_audit() -> dict[str, Any]:
    source = _load_json(RESTORATION_REPLAY)
    rows = [_trial_case(row) for row in source["rows"]]
    accepted_count = sum(row["accepted_by_m4086_policy"] for row in rows)
    decision = (
        "ALGORITHM_V11_THERMO_VALID_CALLSITE_TRIAL_ACCEPTS_ALL_CURATED_CASES"
        if accepted_count == len(rows)
        else "ALGORITHM_V11_THERMO_VALID_CALLSITE_TRIAL_INCOMPLETE"
    )
    return {
        "milestone": "FC4-M4086",
        "audit_schema": "exogibbs_algorithm_v11_thermo_valid_callsite_trial_v1",
        ...
        "callsite_module_path": "src/exogibbs/diagnostics/condensate_algorithm_v11_callsite.py",
        "case_count": len(rows),
        "accepted_case_count": accepted_count,
        "rows": rows,
        "decision": decision,
        "next_default_target": "FC4-M4087",
        "next_default_action": (
            "harden_algorithm_v11_thermo_valid_callsite_policy"
            if accepted_count == len(rows)
            else "debug_algorithm_v11_thermo_valid_callsite_trial"
        ),
    }
...
def _write_markdown(path: Path, compact: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# FC4-M4086 Algorithm-v1.1 Thermo-Valid Callsite Trial",
        "",
        f"- Decision: `{summary['decision']}`",
        f"- Case count: `{summary['case_count']}`",
        f"- Accepted case count: `{summary['accepted_case_count']}`",
        f"- Callsite module: `{summary['callsite_module_path']}`",
    ...
    ledger.update(
        {
            ...
            "fastchem4_current_milestone": "FC4-M4086",
            "fastchem4_algorithm_v11_thermo_valid_callsite_status": audit["decision"],
            "fastchem4_algorithm_v11_thermo_valid_callsite_module_path": audit[
                "callsite_module_path"
            ],
            "fastchem4_algorithm_v11_thermo_valid_callsite_accepted_case_count": audit[
                "accepted_case_count"
            ],
...
def main() -> None:
    ...
    compact = {
        "milestone": "FC4-M4086",
        "campaign_type": "algorithm_v11_thermo_valid_callsite_trial",
        ...
        "audit_summary": {
            ...
            "callsite_module_path": audit["callsite_module_path"],

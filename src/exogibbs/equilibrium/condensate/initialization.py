...
def regauge_gas_only_warm_start(
    ...
    floor. This is a Python-level host utility and returns gas fields only.
    """
    ...
    if not np.any(representing):
        raise ValueError("gas_ln_n cannot be regauged to element_inventory.")
    ...
    target_log_scale = math.log(target_amount) - math.log(represented_amount)
    regauged_log_amounts = np.full(
        expected_gas_shape,
        -np.inf,
        dtype=np.float64,
    )
    regauged_log_amounts[usable] = (
        gas_log_amounts[usable] - reference_log_amount + target_log_scale
    )
    if not np.all(np.isfinite(regauged_log_amounts[usable])):
        raise ValueError("The gas warm-start gauge shift is not finite.")
    log_gas_total = float(np.logaddexp.reduce(regauged_log_amounts[usable]))
    ...
    log_floor = min(relative_log_floor, represented_log_floor)

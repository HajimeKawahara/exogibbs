...
def _build_chemical_setup(
    ...
    coeffs = jnp.asarray(coeffs, dtype=float_dtype)
    t_uppers = jnp.asarray(t_uppers, dtype=float_dtype)
    segment_counts = jnp.asarray(segment_counts, dtype=jnp.int32)
    formula_matrix = jnp.asarray(formula_matrix, dtype=float_dtype)
    element_vector_ref = jnp.asarray(element_vector_ref, dtype=float_dtype)

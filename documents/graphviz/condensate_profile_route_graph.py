"""Generate a Japanese Graphviz diagram for condensate profile execution."""
...
DOT_PATH = HERE / "condensate_profile_route.dot"
PNG_PATH = HERE / "condensate_profile_route.png"
...
DOT_SOURCE = f"""digraph condensate_profile_route {{
  graph [
    ...
    splines=ortho,
    fontname="{FONT}",
    label="ExoGibbs condensate profile 実行経路",
    labelloc=t,
    fontsize=26
  ];
  node [
  ...
  subgraph cluster_batch {{
    ...
    merge_rescue [
      label="accepted rescue rows を merge\\n元の profile shape を保持",
      fillcolor="#f3e8ff"
    ];
    rescued_accepted [
      shape=diamond,
  ...
  subgraph cluster_scan {{
    ...
    scan_loop [
      label="scan_hot layer loop\\nprevious accepted layer を initializer にする",
      fillcolor="#ffedd5"
    ];
    head_route [
    ...
    fresh_fallback [
      label="warm-start failure は\\nfresh one-layer route へ fallback",
      fillcolor="#fed7aa"
    ];
  }}

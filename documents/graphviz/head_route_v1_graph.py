"""Generate a Japanese Graphviz diagram for ExoGibbs HEAD route v1.4."""
...
DOT_SOURCE = f"""digraph head_route_v1 {{
  graph [
    ...
    splines=ortho,
    fontname="{FONT}",
    label="ExoGibbs 凝縮あり HEAD route v1.4",
    labelloc=t,
    fontsize=26
  ];
  node [
  ...
  subgraph cluster_public_result {{
    ...
    v11_fallback_gate [
      shape=diamond,
      label="v1.3 retry gates + v1.4 full-budget gate / budget repair retries\\ncenter / residual / soft-restoration / Ipopt h-type / support-cap\\nrelative joint budget / budget-preserving seed / strict gas",
      fillcolor="#fef3c7"
    ];

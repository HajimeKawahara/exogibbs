"""Generate a Japanese Graphviz diagram for ExoGibbs HEAD route v1.18."""
...
DOT_SOURCE = f"""digraph head_route_v1 {{
  graph [
    ...
    splines=ortho,
    fontname="{FONT}",
    label="ExoGibbs 凝縮あり HEAD route v1.18",
    labelloc=t,
    fontsize=26
  ];
  node [
  ...
  empty_support -> gas_only_budget_gate [label="はい"];
  gas_only_budget_gate -> gas_only_exit [label="はい"];
  gas_only_budget_gate -> strict_gas_retry [label="いいえ"];
  ...
  lifecycle_accepted -> v18_final_gate [label="いいえ"];
